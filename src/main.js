...
function generate_wave_event(wave_num) {
    ...
    if (wave_num >= 10) { // Rares spawn from wave 10+
        make_event(frame + 600, generate_raremonster_event, [wave_num]); 
        make_event(frame + 1200, generate_raremonster_event, [wave_num]); 
        make_event(frame + 1800, generate_raremonster_event, [wave_num]); 
        make_event(frame + 2400, generate_raremonster_event, [wave_num]); 
    }
